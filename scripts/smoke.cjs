...
app.whenReady().then(async () => {
  ...
  const until = async (code, ms = 3000) => { for (let waited = 0; waited < ms; waited += 25) { if (await evaluate(code)) return true; await delay(25); } return false; };
  const mute = async value => { window.webContents.send('mute', value); assert.ok(await until(`muted === ${value}`), `Mute ${value} reaches the renderer`); };
  ...
  await evaluate('document.querySelector("#mute").dispatchEvent(new PointerEvent("pointerdown", { button: 0, bubbles: true }))');
  await delay(600);
  await evaluate('document.querySelector("#mute").dispatchEvent(new PointerEvent("pointerup", { button: 0, bubbles: true })); document.querySelector("#mute").click()');
  assert.equal(await evaluate('!volumePanel.hidden && !muted'), true, 'Long press opens the volume panel without toggling mute');
  await evaluate('volumeSlider.value = 40; volumeSlider.dispatchEvent(new Event("input")); speak()');
  assert.equal(await evaluate('Math.abs(cry.volume - .45 * .4) < 1e-9 && Math.abs(growthAudio.music.volume - .45 * .4) < 1e-9 && localStorage.getItem("volume")'), '40', 'Volume scales cries and growth audio and persists');
  await evaluate('cry.pause(); button.dispatchEvent(new PointerEvent("pointerdown", { button: 0, pointerId: 3, bubbles: true })); button.dispatchEvent(new PointerEvent("pointerup", { button: 0, pointerId: 3, bubbles: true }))');
  await delay(80);
  assert.equal(await evaluate('volumePanel.hidden'), true, 'Pressing outside closes the volume panel');
  await evaluate('applyVolume(100)');
  await mute(true);
  await evaluate('speak()');
  ...
  assert.ok(await until('!growthAudio.level.paused'), 'Level-up effect plays');
